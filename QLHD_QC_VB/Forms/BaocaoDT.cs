...
namespace QLHD_QC_VB.Forms
{
    public partial class frmbaocaodt : Form
    {
        ...
        private void btninbaocao_Click(object sender, EventArgs e)
        {
            ...
            // Sửa ở đây: ép kiểu Worksheets[1] từ object sang Worksheet
            exBook = exApp.Workbooks.Add(COMExcel.XlWBATemplate.xlWBATWorksheet);
            exSheet = (COMExcel.Worksheet)exBook.Worksheets[1]; 

            // Sửa ở đây: ép kiểu Cells[1, 1] từ object sang Range
            ...
            {
                exSheet.Cells[row + 3, 1] = row + 1; // Cột 1 là STT

                for (col = 0; col <= tblbaocaodt.Columns.Count - 1; col++)
                {
                    if (tblbaocaodt.Columns[col].ColumnName == "ngayky")
                    {
                        exSheet.Cells[row + 3, col + 2] = Convert.ToDateTime(tblbaocaodt.Rows[row]["ngayky"]);
                    }
                    else
                    {
                        exSheet.Cells[row + 3, col + 2] = tblbaocaodt.Rows[row][col].ToString();
                    }
                }
            }
        ...
        private void frmbaocaodt_Load_1(object sender, EventArgs e)
        {
