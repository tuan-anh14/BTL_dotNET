    ...
    public partial class frmhopdong : Form
    {
        ...
        private void btnxuathopdong_Click(object sender, EventArgs e)
        {
            ...
            if (txtmahd.Text.Substring(0, 2) == "VB")
            {
                ...
                {
                    exSheet.Cells[row + 28, 1] = row + 1; // Ghi số thứ tự vào cột đầu tiên (A)
            ...
            else
            {
                ...
                {
                    exSheet.Cells[row + 28, 1] = row + 1; // Cột A: STT
                ...
                // ==== Căn dòng bắt đầu cho các điều khoản tiếp theo ====
                int baseRow = row + 29;

                // ==== Điều 2 ====
                exRange = exSheet.Range[$"A{baseRow}:H{baseRow}"];
                exRange.Merge();
                exRange.HorizontalAlignment = COMExcel.XlHAlign.xlHAlignLeft;
                exRange.Value = "ĐIỀU 2: GIÁ TRỊ HỢP ĐỒNG";
                exRange.Font.Bold = true;

                // ==== Điều 2 - Chi tiết ====
                exRange = exSheet.Range[$"A{baseRow + 1}:H{baseRow + 1}"];
                exRange.Merge();
                exRange.HorizontalAlignment = COMExcel.XlHAlign.xlHAlignLeft;
                exRange.Value = "Chi phí cho việc thực hiện nội dung công việc tại Điều 1 của hợp đồng này là: " +
                    tblhd.Rows[0][4].ToString() +
                    " (Bằng chữ: " + Class.Functions.ConvertNumberToString(tblhd.Rows[0][4].ToString()) + ")";
                exRange.WrapText = true;

                // ==== Điều 3 ====
                exRange = exSheet.Range[$"A{baseRow + 2}:H{baseRow + 2}"];
                exRange.Merge();
                exRange.HorizontalAlignment = COMExcel.XlHAlign.xlHAlignLeft;
                exRange.Value = "ĐIỀU 3: Bên A phải trả tiền đầy đủ cho bên B 01 (một) lần bằng tiền mặt và bên A phải cấp cho bên B hợp đồng đầy đủ để có cơ sở thanh toán chi phí cho đợt quảng cáo";
                exRange.Font.Bold = true;
                exRange.WrapText = true;

                // ==== Đại diện Bên A ====
                exRange = exSheet.Range[$"A{baseRow + 3}:C{baseRow + 3}"];
                exRange.Merge();
                exRange.HorizontalAlignment = COMExcel.XlHAlign.xlHAlignCenter;
                exRange.Value = "ĐẠI DIỆN BÊN A";
                exRange.Font.Bold = true;

                exRange = exSheet.Range[$"A{baseRow + 4}:C{baseRow + 4}"];
                exRange.Merge();
                exRange.HorizontalAlignment = COMExcel.XlHAlign.xlHAlignCenter;
                string[] Name_KH = txttenkh.Text.Split(' ');
                exRange.Value = Name_KH.Length >= 3 ? Name_KH[2] : txttenkh.Text;

                exRange = exSheet.Range[$"A{baseRow + 5}:C{baseRow + 5}"];
                exRange.Merge();
                exRange.HorizontalAlignment = COMExcel.XlHAlign.xlHAlignCenter;
                exRange.Value = txttenkh.Text;

                // ==== Đại diện Bên B ====
                exRange = exSheet.Range[$"F{baseRow + 3}:H{baseRow + 3}"];
                exRange.Merge();
                exRange.HorizontalAlignment = COMExcel.XlHAlign.xlHAlignCenter;
                exRange.Value = "ĐẠI DIỆN BÊN B";
                exRange.Font.Bold = true;

                exRange = exSheet.Range[$"F{baseRow + 4}:H{baseRow + 4}"];
                exRange.Merge();
                exRange.HorizontalAlignment = COMExcel.XlHAlign.xlHAlignCenter;
                string[] Name_NV = txttennv.Text.Split(' ');
                exRange.Value = Name_NV.Length >= 3 ? Name_NV[2] : txttennv.Text;

                exRange = exSheet.Range[$"F{baseRow + 5}:H{baseRow + 5}"];
                exRange.Merge();
                exRange.HorizontalAlignment = COMExcel.XlHAlign.xlHAlignCenter;
                exRange.Value = txttennv.Text;
